...
void MessUnits(wchar_t *Head){ IsSyntError=true;
 swprintf(Inf,L"Элементом списка инструкции \"%s\"\n"
  L"должен быть тип размерности:\n"
  L" \'m\', \'l\', \'t\' или \'a\'\n"
  L"соответственно масса, длина, время или угол\n"
  L"и в скобках после него - предпочтительная в этом\n"
  L"файле пользовательская размерность из доступных:\n"
  L"\"г\", \"кг\", \"т\",\n"
  L"\"мм\", \"см\", \"м\", \"км\", \"миля\", \"дм\", \"дюйм\",\n"
  L"\"с\", \"мин\", \"час\", \"день\", \"месяц\", \"год\",\n"
  L"\"рад\", \"°\"");
 Application->MessageBoxA(Inf,L" ОШИБКА ",MB_OK|MB_ICONERROR);
}
//---------------------------------------------------------------------------
...
MB_ICONWARNING);
 ...
 if((I=FindInstLast(L"ЕДИНИЦЫ ИЗМЕРЕНИЯ"))||
  ...
  List *Lold=L; L=TakeList(L"РАЗМЕРНОСТИ");
  ...
  switch(P->Nam[0]){
   ...
   case 'l':
    if(wcscmp(P->R->Nam,L"мм")  &&wcscmp(P->R->Nam,L"см")&&
     ...
     wcscmp(P->R->Nam,L"миля")&&wcscmp(P->R->Nam,L"дм")&&
    ...
    break;
   case 't':
    if(wcscmp(P->R->Nam,L"с")  &&wcscmp(P->R->Nam,L"месяц")&&
