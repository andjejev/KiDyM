...
void Vvod(TRichEdit *RE,TCGauge *CGauge,TStatusBar *StatusBar,
 ...
 int Flag,N,Ln,Rez,LineComm,C; Arra *A; Vary *V; Form F; Body *b; Parm *P; Cord *K;
...
//    }
 ...
 case TEXTCOMM:
   ...
   t->Atr=TEXTCOMM; t->S=char(B); LineComm=Line;
   if(B=='#') C=1; else C=2;
   while(ReadChar()){
    t->S+=char(B);
    switch(B){
     case '#':
      if(C==1){
         if(ImageKDM)ImageKDM->Add(t,LineComm);
       goto L; 
      }
      break;
     case '{': if(C!=1) C+=2; break;
     case '}':
      C-=2;
      if(!C){
       if(ImageKDM) ImageKDM->Add(t,LineComm);
...
//---------------------------------------------------------------------------
/*
void __fastcall TImageKDM::Sizes(TImage *Img, int y)  {
 int i,str,H,L,HM=Img->Canvas->TextHeight("M");
...
//заполним для непустых строк требуемую высоту и ширину
 for(r=R; r; r=r->Sled){
  i=r->Line-1; H=r->Height(Img);
  L=r->Width(Img,SL);
  if(H>SL[i].h) SL[i].h=H;
  SL[i].l+=L;
 }
//укажем величину отступа по вертикали для каждой строки
 for(i=0; i<str; i++){
  SL[i].y=y; y+=SL[i].h;
}}*/
//---------------------------------------------------------------------------
