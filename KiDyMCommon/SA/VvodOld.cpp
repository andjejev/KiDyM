//#include "D:\Study\Andreev\E\Aju\CiDyMnew\Kinematika\BC5\SAV\CA.h"
#include "CA.h"
#include "ConvDW.h"
#include <string.h>
#include <alloc.h>
   ...
   if(s=strrchr(Dir,'.')){
    ...
    if(!strcmp(Ext,"BMP")||!strcmp(Ext,"JPG")||!strcmp(Ext,"JPEG")){ int S1,S2;
     ...
     if(!strcmp(Ext,"BMP")){
      ...
      Rs=Image->Canvas->ClipRect;
//Вычислим абсциссы
      Rd.Left=(pbReport->Canvas->ClipRect.Width()-Rd.Width())/2;
      Rd.Right+=Rd.Left; Rd.Top=y; Rd.Bottom+=y; y+=Rd.Height();
//      Rs.Left=pbReport->Canvas->ClipRect.Top+Rs.Height();
      //(pbReport->Canvas->ClipRect.Height()-Rs.Height())/100 ;// >Canvas->ClipRect.Height());
//      Rs.Right+=Rs.Left;
//Здесь надо вычислить ординаты по текущей вертикальной позиции, а
//1. Исходные данные писать после рисунка!
      pbReport->Canvas->CopyRect(Rd,Image->Canvas, Rs);// (0,0,Image);// >Canvas->Draw(0,0,Image);
    }}
    else if(!strcmp(Ext,"JPG")||!strcmp(Ext,"JPEG")){
     TJPEGImage *jp = new TJPEGImage();
