...
//---------------------------------------------------------------------
#pragma resource "*.dfm"
TAboutFormKin *AboutFormKin;
extern wchar_t Inf[]; extern int NYear;
//---------------------------------------------------------------------
__fastcall TAboutFormKin::TAboutFormKin(TComponent* AOwner):TForm(AOwner){
// ColorShar=(TColor)(unsigned long)(255l+200l*256l);
 ColorShar=(TColor)(unsigned long)(255l+128l*256l+64l*256l*256l);
 ColorLine=clBlue;
}
//---------------------------------------------------------------------
void __fastcall TAboutFormKin::DrawShar(TCanvas *Canvas,int X,int Y){
 Canvas->Pen->Color=ColorShar/2;
 Canvas->Ellipse(X-r,Y-r,X+r,Y+r);
 Canvas->Pen->Color=ColorShar;
 Canvas->Ellipse(X-14*r/16,Y-14*r/16,X+14*r/16,Y+14*r/16);
 Canvas->Pen->Color=(TColor)(unsigned long)(255l+200l*256l+128l*256l*256l);
 Canvas->Ellipse(X+3,Y-7,X+7,Y-3);
 Canvas->Pen->Color=(TColor)(unsigned long)(255l+227l*256l+192l*256l*256l);
 Canvas->Ellipse(X+2,Y-8,X+8,Y-2);
}
//---------------------------------------------------------------------
void __fastcall TAboutFormKin::DrawTetraedr(double Fi,double Psi,double Teta){
 ...
 if(Z2>Z1){
  X=X1; Y=Y1; Z=Z1; X1=X2; Y1=Y2; Z1=Z2; X2=X; Y2=Y; Z2=Z;
 }
 X3=x3*cos11+y3*cos12+z3*cos13;
 Y3=x3*cos21+y3*cos22+z3*cos23;
 Z3=x3*cos31+y3*cos32+z3*cos33;
 if(Z3>Z2){
  X=X2; Y=Y2; Z=Z2; X2=X3; Y2=Y3; Z2=Z3; X3=X; Y3=Y; Z3=Z;
 }
 if(Z2>Z1){
  X=X1; Y=Y1; Z=Z1; X1=X2; Y1=Y2; Z1=Z2; X2=X; Y2=Y; Z2=Z;
 }
 ...
 if(Z4>Z3){
  X=X3; Y=Y3; Z=Z3; X3=X4; Y3=Y4; Z3=Z4; X4=X; Y4=Y; Z4=Z;
 }
 if(Z3>Z2){
  X=X2; Y=Y2; Z=Z2; X2=X3; Y2=Y3; Z2=Z3; X3=X; Y3=Y; Z3=Z;
 }
...
//---------------------------------------------------------------------
void __fastcall TAboutFormKin::FormShow(TObject *Sender){ wchar_t *s;
 t=0; dt=Timer->Interval; l=140; r=16; a=r/l; b=1-a;
// Width=450; Height=400; Caption=L"Программа \"Кинематика КИДИМ\"";
 ...
 y1=-l/2.0; y2=-y1; y3=y4=0.0;
 z1=z2=z3=-h/4.0; z4=-3*z1;
 ...
 for(int i=0;i<12;i++){
//  BufferIcon->Canvas->Ellipse(Xm[i]-1,Ym[i]-1,Xm[i]+1,Ym[i]+1);
 ...
 Timer->Enabled=true;
 wcscpy(Inf,Label4->Caption.c_str());
 if(s=wcschr(Inf,'-')) swprintf(s+1,L"%d",NYear+1900);
 Label4->Caption=Inf;
}
//---------------------------------------------------------------------------
void __fastcall TAboutFormKin::TimerTimer(TObject *Sender){
 t+=dt/1000.0;
...
//---------------------------------------------------------------------------
void __fastcall TAboutFormKin::HyperLinkMouseMove(TObject *Sender,
      TShiftState Shift, int X, int Y){
 HyperLink->Font->Color=clRed;
 HyperLink->Cursor=crHandPoint;
}
//---------------------------------------------------------------------------
void __fastcall TAboutFormKin::HyperLinkClick(TObject *Sender){
 AnsiString doc=L"mailto:"+HyperLink->Caption;
 ShellExecute(NULL,"open",doc.c_str(),NULL,NULL,SW_SHOW);
}
//---------------------------------------------------------------------------
void __fastcall TAboutFormKin::FormMouseMove(TObject *Sender,
      TShiftState Shift, int X, int Y){
 HyperLink->Font->Color =clBlue; HyperLink->Cursor =crDefault;
}
//---------------------------------------------------------------------------
