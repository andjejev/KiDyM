...
//Занесение текста в панель состояния
 if(Progress>0)
  StatusBar->SimpleText=Format("Документ %s: прочитано %d Кбайт из %d",
...
void __fastcall TFormOtchet::tbHelpClick(TObject *Sender){
 ...
 if(s=strrchr(Help,'\\')) strcpy(s+1,"Help\\BrouserHelp.htm");
 else strcat(Help,"\\Help\\BrouserHelp.htm");
 if(access(Help,0)){
